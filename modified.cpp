...
//Cuboid properties
float cuboidSize = 2.0f;
float cuboidX = 0.0f;
float cuboidY = 5.0f;
float cuboidZ = 7.0f;
float cuboidSpeed = 0.01f;
...
void drawcuboid()
{
    glPushMatrix();
    glPushMatrix();
    glTranslatef(cuboidX, cuboidY, cuboidZ);
    glRotatef(angle,1.0,1.0,1.0);
    glColor3f(1.0f, 0.750f, 0.0f);
    glutSolidCube(cuboidSize);
    glPopMatrix();

    glPushMatrix();
    glPushMatrix();
    glTranslatef(2.0f,5.0f,7.0f);
    glRotatef(angle,1.0,1.0,1.0);
    glColor3f(1.0f, 0.750f, 0.0f);
    glutSolidCube(cuboidSize);
    glPopMatrix();
...
void reshape(int width, int height)
{
    ...
    glLoadIdentity();
    gluPerspective(45.0f, (float)width / (float)height, 0.1f, 100.0f); //viewing angles,znear,zfar
}
...
std::string rules = "Keyboard Interactions: Press the arrow keys to move the board";
std::string rule = " along +ve axis and -ve x-axis.";
...
void display2(void) {
    ...
    renderText(0.05f, 0.75f, GLUT_BITMAP_HELVETICA_18, rule);
...
int main(int argc, char** argv)
{
    ...
    glutAddMenuEntry("Draw Wire Sphere", 4);
     glutAddMenuEntry("Draw Cuboid", 5);
